...
class Profile(APIView):
    def get(self, request, id):
        ...
        feed_list = Feed.objects.filter(email = user.email).order_by("-id")
...
class UserFeed(APIView):
    def get(self, request, id):
        ...
        feed_object_list = Feed.objects.filter(user_id = id).order_by('-id')
        feed_list = []
        for feed in feed_object_list:
            user = User.objects.filter(email = feed.email).first()
            login = User.objects.filter(email = email).first()
        ...
        return render(request, "content/feed.html",context = dict(feeds = feed_list, user = user, follows = follow_list))